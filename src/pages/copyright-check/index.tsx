...
export default function CopyrightCheck() {
  ...
  useEffect(() => {
    ...
    // Lade Draft aus Firebase wenn ID in Query
    const loadDraft = async () => {
      const editId = router.query.edit as string
      if (editId && auth.currentUser) {
        try {
          const docRef = doc(db, 'checked_products', editId)
          ...
          if (docSnap.exists()) {
            ...
            setIsAICreated(data.isAICreated ?? null)
            setHasHumanCreativity(data.hasHumanCreativity ?? null)
            ...
            setIsCommercial(data.isCommercial ?? null)
            setResult(data.result || null)
            
            // Gehe zu Step 7 (letzter Schritt)
            setStep(7)
          }
        } catch (err) {
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        {showHistory ? (
          ...
          <>
                ...
                <div className="flex space-x-4 mt-6">
                  ...
                  <button
                    onClick={async () => {
                      ...
                      // Speichere als DRAFT in Firebase
                      try {
                        const isProtected = !isPublicDomain && !hasCCLicense
                        
                        const docRef = await addDoc(collection(db, 'checked_products'), {
                          userId: auth.currentUser.uid,
                          mediaType,
                          isAICreated,
                          hasHumanCreativity,
                        ...
                        // Navigiere zur Result-Seite mit ID
                        router.push(`/copyright-check/result?id=${docRef.id}`)
                      } catch (err) {
            ...
            {/* Step 8 removed - now using /copyright-check/result page */}
          </>
        )}
      </main>
    </div>
